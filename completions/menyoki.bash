_menyoki() {
    ...
    case "${cmd}" in
        ...
        menyoki__analyze__save)
            ...
            ;;
        menyoki__capture)
            opts=" -r -f -h -V -k -b -p -s -d -c -t -i  --root --focus --select --with-alpha --no-keys --help --version --keys --border --padding --size --duration --countdown --timeout --interval --font  <COMMAND>  png jpg bmp ico tiff tga pnm ff save help  out"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
        ...
        menyoki__misc)
            ...
            ;;
        menyoki__record)
            opts=" -r -f -h -V -k -b -p -s -d -c -t -i  --root --focus --select --with-alpha --no-keys --help --version --keys --border --padding --size --duration --countdown --timeout --interval --font  <COMMAND>  gif apng save help    out"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
